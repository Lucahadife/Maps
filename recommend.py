...
def find_closest(location, centroids):
    ...
    def distance (location, centroids):
        distances = []
        for centroid in centroids:
            distances.append(sqrt((location[0] - centroid[0]) ** 2 + (location[1] - centroid[1]) ** 2))
        return distances
    
    distances = distance(location, centroids)
...
def find_centroid(cluster):
    ...
    """
    locs = [restaurant_location(cluster[i]) for i in range(len(cluster))]

    x_cord = [x[0] for x in locs]
    y_cord = [x[1] for x in locs]

    lat_mean = sum(x_cord) / len(x_cord)
    long_mean = sum(y_cord) / len(y_cord)

    centroid = [lat_mean, long_mean]
    return centroid
...
def best_predictor(user, restaurants, feature_fns):
    ...
    predictors_and_r_squared = []
    
    for feature_fn in feature_fns:
        predictor, r_squared = find_predictor(user, reviewed, feature_fn)
        predictors_and_r_squared.append((predictor, r_squared))
    
    best_predictor, _ = max(predictors_and_r_squared, key=lambda x: x[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    return {restaurant_name(restaurant): int(user_score(user, restaurant_name(restaurant))) 
            if any(restaurant_name(r) == restaurant_name(restaurant) for r in reviewed) 
            else float(predictor(restaurant)) 
...
def search(query, restaurants):
    ...
    """
    return list(filter(lambda restaurant: query in restaurant_categories(restaurant), restaurants))
